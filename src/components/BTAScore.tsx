export function BTAScore({ score, size = 'md' }: { score: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  const scoreColor = score >= 90 ? 'var(--color-bta-primary)' : score >= 75 ? 'var(--color-bta-amber)' : 'var(--color-bta-error)'

  return (
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: dim, height: dim }}>
        <svg width={dim} height={dim} style={{ transform: 'rotate(-90deg)' }}>
          <circle cx={dim / 2} cy={dim / 2} r={r} fill="none" stroke="var(--color-bta-border)" strokeWidth={sw} />
