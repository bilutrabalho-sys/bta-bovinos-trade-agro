...
export function CreateListingScreen({ onBack }: { onBack: () => void }) {
  ...
  const stepContent = [
    ...
    <div key={1} className="space-y-3"><p className="text-bta-muted text-sm">Um vídeo curto aumenta em 3× o interesse no lote.</p><div className="h-40 bg-bta-bg border-2 border-dashed border-bta-border rounded-xl flex flex-col items-center justify-center gap-2 text-bta-muted"><span className="text-3xl">▶️</span><p className="text-xs">Adicionar vídeo</p></div></div>,
    ...
    <div key={5} className="space-y-4"><div><label className="text-bta-text text-xs font-display font-semibold block mb-1">Cidade / Estado</label><input type="text" placeholder="Ex: Barretos, SP" className="w-full bg-bta-surface border border-bta-border rounded-xl px-4 py-3 text-sm outline-none" /></div><div className="bg-bta-bg rounded-xl h-36 flex items-center justify-center text-bta-muted text-sm border border-bta-border">📍 Confirmar no mapa</div></div>,
    ...
    <div key={9} className="flex flex-col items-center gap-4 py-8"><div className="w-20 h-20 bg-bta-success/10 rounded-full flex items-center justify-center text-4xl">✅</div><h2 className="font-display font-black text-bta-text text-xl text-center">Lote publicado!</h2><p className="text-bta-muted text-sm text-center">Visível para compradores em todo o Brasil.</p><div className="bg-bta-bg rounded-xl p-3 w-full border border-bta-border text-center"><p className="text-bta-muted text-xs">BTA Score do anúncio</p><p className="font-display font-black text-bta-primary text-3xl mt-1">87</p></div></div>,
  ]
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <Header title="Novo anúncio" onBack={onBack} rightAction={<button className="text-bta-muted text-xs font-display font-semibold">Salvar rascunho</button>} />
      ...
      <div className="px-5 pb-8 pt-4 bg-bta-surface border-t border-bta-border flex gap-3">
        ...
        <button onClick={() => step < steps.length - 1 ? setStep(s => s + 1) : onBack()} className="flex-1 bg-bta-primary text-white font-display font-bold text-base py-4 rounded-2xl">
