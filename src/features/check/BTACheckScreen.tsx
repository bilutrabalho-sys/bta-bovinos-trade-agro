...
import { Header, BTAScore } from '@/components'

export function BTACheckScreen({ onBack, onNavigate }: { onBack: () => void; onNavigate: (s: Screen) => void }) {
  const items = [
    { label: 'Dados do vendedor', status: 'ok', note: 'Fazenda BTA Verified, 4.9 ★' },
  ...
  const statusConfig = {
    ok: { label: 'OK', bg: 'bg-bta-success/10', text: 'text-bta-success', icon: '✓' },
    verify: { label: 'VERIFICAR', bg: 'bg-bta-amber/10', text: 'text-bta-amber', icon: '?' },
    attention: { label: 'ATENÇÃO', bg: 'bg-bta-error/10', text: 'text-bta-error', icon: '!' },
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
        <div>
          ...
          <p className="text-bta-muted text-sm">120 Nelore · Fazenda Santa Helena</p>
        </div>
            ...
            return (
              <div key={item.label} className="bg-bta-surface rounded-xl border border-bta-border p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`px-2 py-0.5 rounded-lg text-[10px] font-display font-bold flex-shrink-0 ${cfg.bg} ${cfg.text}`}>{cfg.icon} {cfg.label}</span>
                </div>
              </div>
            )
          })}
      ...
      <div className="px-5 pb-8 pt-4 bg-bta-surface border-t border-bta-border space-y-3">
        <button onClick={() => onNavigate('negotiation')} className="w-full btn-primary-grad text-white font-display font-bold text-base py-4 rounded-2xl">Continuar para negociação</button>
