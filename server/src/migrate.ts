...
//
//  Uso:
//    npm run db:setup             -> aplica o que faltar + re-seed
//    npm run db:setup -- --reset  -> dropa e recria o schema public antes (dev)
// ============================================================================
...
const SEED = path.join(DB_DIR, 'seed', 'seed.sql');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 4. Seed (idempotente: truncate+insert) — sempre.
    if (existsSync(SEED)) {
      await runFile(pool, SEED, 'seed/seed.sql');
    } else {
      console.warn('[db:setup] aviso: seed.sql não encontrado, pulando.');
    }
